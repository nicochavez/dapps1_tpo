...
export default function ItemDetailLiveView({ item, parentCatalog, navigation }) {
  ...
  const currentPrice = item?.currentBid || item?.precioBase || 0;
  ...
  const handlePlaceBid = () => {
    if (lotState !== 'en_puja') {
      alert('This lot is not currently open for bidding.');
      return;
    }

    if (!bidAmount || isNaN(bidAmount) || Number(bidAmount) <= currentPrice) {
